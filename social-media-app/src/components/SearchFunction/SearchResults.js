...
const SearchResults = () => {
  ...
  useEffect(() => {
    if (query) {
      ...
      axios
       .get(`https://nediecom.onrender.com/search?q=${encodeURIComponent(query)}`)
        // .get(`http://localhost:10000/search?q=${encodeURIComponent(query)}`)
        .then(res => {
          setResults(res.data)
          setLoading(false);
        })
          ...
          {results.map((p) => (
            <Link to={`/product/${p._id}`}>
                ...
                <div className='flex-child1' >
                  ...
                  <h5>£{p.productprice.toFixed(2)}</h5>
                  </div>
